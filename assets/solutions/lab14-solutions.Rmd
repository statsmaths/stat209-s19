...
```{r}
ggplot(mpg, aes(displ, hwy)) +
  geom_point(size = 2, color="olivedrab")
```
...
ggplot(mpg, aes(displ, hwy)) +
  ...
  ylab("Miles Per Gallon") +
  xlab("Engine Size") +
  ggtitle("Plot of automobile fuel efficency")
```
...
```{r}
ggplot(burrito, aes(google, yelp)) +
  geom_point() +
  geom_bestfit()
```
...
```{r}
tmod_linear_regression(yelp ~ google, data = burrito)
```
