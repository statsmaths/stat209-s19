...
on the x-axis and the actual maximum temperatures on the y-axis. Make sure
...
minimum temperatures and the actual maximum temperatures. Date is still
on the x-axis. Further, color the minimum temperatures with the color
"#5be5e5" (it's a pretty shade of blue) and the maximum temperatures
...
x and y aesthetics control where the segment begins, but you must also specify
...
segments that start with a y-value of actual_max_temp and proceed down to the
y-value of actual_in_temp. That is, each day will have a line going from the high
temperature to the low temperature. Further, the segments should be colored according
to the actualy_mean_temp and you should add this layer:

- scale_color_continuous(low = "#5be5e5", high ="#ff6666")

To the plot to make the temperatures show up on a nice-looking scale (cold temperatures
are blue and warm temperatures are read).
